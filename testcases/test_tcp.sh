...
# Test script for TCP protocol in neofrp
# This script tests both TCP and UDP forwarding through TCP transport
# Note: WebSocket (ws/wss) is documented but not yet implemented in the validation code
...
# Print header
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  TCP Protocol Test${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${YELLOW}Note: WebSocket (ws/wss) is documented but not yet${NC}"
echo -e "${YELLOW}implemented in the validation code. Using TCP protocol.${NC}"
echo ""
...
# Create TCP protocol config files
echo -e "${YELLOW}Creating TCP protocol configuration files...${NC}"

cat > "$SCRIPT_DIR/server_tcp_test.json" <<EOF
{
    "log": {
        "log_level": "debug"
    },
...
cat > "$SCRIPT_DIR/client_tcp_test.json" <<EOF
{
    "log": {
        "log_level": "debug"
    },
...
# Step 3: Start FRP server with TCP
echo -e "${YELLOW}[3/6] Starting FRP server with TCP protocol...${NC}"
"$BIN_DIR/frps" -c "$SCRIPT_DIR/server_tcp_test.json" > "$LOG_DIR/frps.log" 2>&1 &
SERVER_PID=$!
sleep 3
...
# Step 4: Start FRP client with TCP
echo -e "${YELLOW}[4/6] Starting FRP client with TCP protocol...${NC}"
"$BIN_DIR/frpc" -c "$SCRIPT_DIR/client_tcp_test.json" > "$LOG_DIR/frpc.log" 2>&1 &
CLIENT_PID=$!
sleep 3
...
echo -e "${YELLOW}[5/6] Testing TCP forwarding through TCP transport...${NC}"
...
echo -e "${YELLOW}[6/6] Testing UDP forwarding through TCP transport...${NC}"
...
if [ $TCP_TEST_RESULT -eq 0 ] && [ $UDP_TEST_RESULT -eq 0 ]; then
    ...
    echo -e "${GREEN}TCP protocol is working correctly${NC}"
    EXIT_CODE=0
else
